...
def _patched_runtime(
    ...
    def _get_secret(name: str) -> str:
        ...
        if callable(orig_get_secret):
            return orig_get_secret(name)  # type: ignore[misc]
    ...
    def _http_get_json(url: str, params: Optional[Dict[str, Any]] = None, timeout: float = 10) -> Any:
        ...
        if callable(orig_http_get):
            return orig_http_get(url, params=params, timeout=timeout)  # type: ignore[misc]
    ...
    def _http_post_json(url: str, data: Any, timeout: float = 10) -> Any:
        ...
        if callable(orig_http_post):
            return orig_http_post(url, data=data, timeout=timeout)  # type: ignore[misc]
        raise AttributeError("jarvis_runtime.http_post_json is not available")

    try:
        if orig_get_secret is not None:
            jarvis_runtime.get_secret = _get_secret  # type: ignore[assignment]
        if orig_http_get is not None:
            jarvis_runtime.http_get_json = _http_get_json  # type: ignore[assignment]
        if orig_fetch is not None:
            jarvis_runtime.fetch = _http_get_json  # type: ignore[assignment]
        if orig_http_post is not None:
            jarvis_runtime.http_post_json = _http_post_json  # type: ignore[assignment]
        yield
    finally:
        if orig_get_secret is not None:
            jarvis_runtime.get_secret = orig_get_secret  # type: ignore[assignment]
        if orig_http_get is not None:
            jarvis_runtime.http_get_json = orig_http_get  # type: ignore[assignment]
        if orig_fetch is not None:
            jarvis_runtime.fetch = orig_fetch  # type: ignore[assignment]
        if orig_http_post is not None:
            jarvis_runtime.http_post_json = orig_http_post  # type: ignore[assignment]
    ...
    for node in ast.walk(tree):
        ...
        if isinstance(node, ast.ImportFrom):
            if node.module == "jarvis_runtime":
                # NOTE: from jarvis_runtime import X は "jarvis_runtime" 名を作らない場合があるので、
                # ここは "imports_jarvis_runtime=True" として扱う（品質ゲート上は許容）。
                info["imports_jarvis_runtime"] = True
    ...
    for i, t in enumerate(tests):
        ...
        try:
            with _patched_runtime(
                ...
                out = mod.run(params)  # type: ignore[misc]
...
def _score(
    ...
    if ast_info.get("uses_params_get") or (not ast_info.get("uses_params_index")):
        ...
        stability += 4
        warnings.append("run() uses params['x'] indexing; consider params.get(...) for robustness.")
...
def gate(code: str) -> GateResult:
    ...
    # ============================
    # Missing import early exit (B)
    # ============================
    jr_used = bool(ast_info.get("uses_http_get") or ast_info.get("uses_http_post") or ast_info.get("uses_get_secret"))
    if jr_used and not ast_info.get("imports_jarvis_runtime"):
        return GateResult(
            ok=False,
            errors=["jarvis_runtime is used but not imported. Add `import jarvis_runtime` at the top."],
            warnings=[],
            tests=[],
            score=0,
            score_breakdown={"note": "missing import jarvis_runtime"},
        )

    # ============================
    # Missing get_secret early exit
    # ============================
    if ast_info.get("uses_get_secret") and not hasattr(jarvis_runtime, "get_secret"):
        return GateResult(
            ok=False,
            errors=["jarvis_runtime.get_secret is not implemented. Please update jarvis_runtime.py."],
            warnings=[],
            tests=[],
            score=0,
            score_breakdown={"note": "missing jarvis_runtime.get_secret"},
        )
    ...
    try:
        ...
        return GateResult(
            ok=False,
            errors=[f"Import/Load failed: {e}"],
            warnings=[],
            tests=[],
            score=0,
            score_breakdown={"note": "load failed"},
        )
